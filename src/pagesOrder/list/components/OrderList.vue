...
import { useDebounce } from '@/services/debonce'
...
const getOrderListData = async () => {
  const res = await getMemberOrder(params)
...
const onOrderConfirm = (id: string) => {
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        const res = await putMemberOrderByidReceipt(id)
        console.log(res)
...
//   () => props.currentPage,
//   (newIndex) => {
//     params.orderState = newIndex
//     console.log('父组件中的index改变了')
...
<template>
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="order in orderListData" :key="order.id">
      <!-- 订单信息 -->
      <view class="status">
      ...
      <view class="action">
        ...
        <template v-if="order.orderState === OrderState.DaiFuKuan">
          <view @tap="onPay(order.id)" class="button primary">去支付</view>
        </template>
        <template v-else>
          <navigator
