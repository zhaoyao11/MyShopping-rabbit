...
import { onLoad, onReady, onShow, onTabItemTap } from '@dcloudio/uni-app'
import { ref } from 'vue'
import pageSkelon from '../components/pageSkelon.vue'
import { orderStateList, OrderState } from '@/services/constants'
import { getMemberAddressByIdAPI } from '@/services/addressapi'
...
//获取物流信息
const postInfoList = ref<LogisticItem[]>([])
const getPostInfomation = async () => {
  // const res = await getMemberOrderByIdLogisticsAPI(query.id)
  // postInfoList.value = res.result.list
}
//删除订单
const onOrderDelete = () => {
  uni.showModal({
...
<template>
  ...
  >
    <template v-if="order">
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in postInfoList" :key="item.id" class="item">
          <view class="message">
            {{ item.text }}
          </view>
      ...
      <view class="goods">
        ...
        <view class="total">
          <view class="row">
            ...
            <view class="symbol">{{ order.payMoney.toFixed(2) }}</view>
          </view>
          <view class="row">
          ...
          <view class="row">
            ...
            <view class="symbol primary">{{ order.totalMoney.toFixed(2) }}</view>
          </view>
        </view>
      </view>
