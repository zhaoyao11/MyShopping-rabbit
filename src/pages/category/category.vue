...
import { getCategoryTopAPI } from '@/services/categoryapi'
import { getHomeBannerAPI } from '@/services/homeapi'
...
//提取二级分类数据
const getCategoryChildData = computed(() => {
  return CategoryTopList.value[activeIndex.value]?.children || []
})
</script>

<template>
  <view v-if="isFinished" class="viewport">
    <!-- 搜索框 -->
    <view class="search">
      <view class="input">
    ...
    <view class="categories">
      ...
      <scroll-view class="secondary" scroll-y>
        <!-- 焦点图 -->
        <XtxSwiper class="banner" :list="BannerDataList" />
        <!-- 内容区域 -->
        <view class="panel" v-for="item in getCategoryChildData" :key="item.id">
          <view class="title">
            <text class="name">{{ item.name }}</text>
