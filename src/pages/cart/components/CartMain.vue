...
const onChangeCount = (event: InputNumberBoxEvent) => {
  putMemberCartBySkuId(event.index, { count: event.value })
}
...
const selectedCartList = computed(() => {
  return CartListData.value.filter((v) => v.selected)
})
//计算选中单品的总数量
...
const gotoPayment = () => {
  ...
  //跳转到结算页
  // uni.showToast({ icon: 'none', title: '正在跳转' })
  ...
  <scroll-view @scrolltolower="onScrollToLower" scroll-y class="scroll-view">
    ...
    <template v-if="memberstore.profile">
      ...
      <view class="cart-list" v-if="CartListData.length">
        ...
        <uni-swipe-action>
          <!-- 滑动操作项 -->
          <uni-swipe-action-item v-for="item in CartListData" :key="item.skuId" class="cart-swipe">
            <!-- 商品信息 -->
            <view class="goods">
              <!-- 选中状态 -->
              <text
